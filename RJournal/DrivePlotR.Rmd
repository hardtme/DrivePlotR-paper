...
The outline of this article is as follows. We describe the relevant methodologies brought together in \pkg{DrivePlotR} in [Section 2](#background). In [Section 3](#plotmap), we discuss all the plot map components shown in Figure \@ref(fig:teaser). Then, we highlight some use cases of \pkg{DrivePlotR} in [Section 4](#usecases) and demonstrate a typical \pkg{DrivePlotR} workflow in [Section 5](#workflow). Lastly, we discuss possible extensions for future versions of \pkg{DrivePlotR} in [Section 6](#extensions).
...
i.  Make the drive data available in the R session. We make non-sensitive drives from a naturalistic driving study available as part of the \pkg{DrivePlotR} package, but any other data with both geographic and temporal components would also be suitable. Recall that \pkg{DrivePlotR} expects one row for each observation in the data.
