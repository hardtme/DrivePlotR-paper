...
To create a \pkg{DrivePlotR} plot map, we bring together interactive elements from \CRANpkg{leaflet} [@chengLeafletCreateInteractive2024] and \CRANpkg{plotly} [@carson]. The maps created by \pkg{DrivePlotR} retain the interactive features of Leaflet maps, including zooming and a selection box for highlighting points. Similarly, the companion graphs have the attributes from a \CRANpkg{plotly} subplot, like shared axes and linked highlighting, zooming, and panning across graphs. We will discuss this in more detail in [Section 3.2](#companions). 
...
As mentioned in [Section 1](#introduction), we created \pkg{DrivePlotR} with the goal of producing linked interactive vehicle trajectory plots that can be easily shared with collaborators while still maintaining data privacy. We achieved this goal by utilizing the interactivity provided by \CRANpkg{crosstalk} [@chengCrosstalkInterWidgetInteractivity2023]. Because of \CRANpkg{crosstalk}, we can save \pkg{DrivePlotR} plot maps within self-contained HTML files while still retaining all of the interactive functionality of the visuals. We can share these HTML files with collaborators, who will still be able to use the interactive features of the plot map from within the HTML file without the need for external hosting.
...
In this section, we describe the components that make up \pkg{DrivePlotR} plot maps. We discuss the map component in [Section 3.1](#map) and the companion graph component in [Section 3.2](#companions).
